...
func callGeminiChat(apiKey, question, context string) (string, error) {
	url := fmt.Sprintf("https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=%s", apiKey)
	prompt := fmt.Sprintf("You are Kather Baksho's friendly AI Nursery Assistant. Answer this plant question concisely in warm English/Bengali context: %s (Plant: %s)", question, context)
